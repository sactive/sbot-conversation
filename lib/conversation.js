...
class Conversation extends EventEmitter {
   ...
   */
  start(msg) {
    this.robot.logger.debug(`Conversation: ${this.id} is beginning ...`);
    let callbacks = [];
    if (this.conversationSchema) {
      ...
      return series(callbacks, err => {
        if (!err) {
          ...
          this.msg.send(`@${msg.message.user.name} ${completeMessage}`);
          ...
          this.msg.send(`@${msg.message.user.name} ${errMsg}`);
        }
      });
    }
  }
  ...
  addObjectQuestion(step, done) {
    this.addChoice(/.*/i, dialogMessage => {
      ...
      try {
        ...
        this.msg.send(`@${this.msg.message.user.name} ${this.lastReply}`);
        return;
      }
  ...
  validateInput(step, input, done) {
    if (step.answer.validation && _.isPlainObject(step.answer.validation)) {
      ...
      if (!result.status) {
        this.msg.send(`@${this.msg.message.user.name} Invalid value. \n${result.message}`);
      } else {
        this.updateAnswers(input, step.entityName);
        done();
      }
  ...
  _invokeDialog(msg, step, done) {
    ...
    this.msg.send(`@${this.msg.message.user.name} ${question}`);
