...
class ConversationManager {
  ...
  addConversation(conversation) {
    ...
    this.robot.logger.debug(`Create a new conversation: ${conversation.id}, name: ${conversation.name}`);
    this.conversationMappings.set(conversation.id, conversation);

    conversation.on('end', final => {
      this.robot.logger.debug(`Conversation: ${conversation.id} end`);
      ...
      conversation.msg.send(`@${conversation.msg.message.user.name} ${response}`);
    });

    conversation.on('expired', () => {
      this.robot.logger.debug(`Conversation: ${conversation.id} is expired`);
    ...
    conversation.on('close', () => {
      this.robot.logger.debug(`Conversation: ${conversation.id} is closed`);
      this.cancelConversation(receiverUserId, conversation.id, options);
    });
